...
void loop() {
  ...
  for (int i=0;i<tam_clave;i++) {
	...
	digitalWrite(blueStatusPin, 0);     	// Enciende el led azul
	...
	if (status==WL_CONNECTED) {				// Si el estado es conectado
		digitalWrite(greenStatusPin, 255);  // Apaga el led verde
		digitalWrite(redStatusPin, 0);     	// Enciende el led rojo
		datos="Conectado!!";
		datos+="PWD=";
		datos+=pwd[i];
		updateTFT();						//Saca por pantalla "Conectado!!" y la clave de conexion
		break;
	}
  }
